...
class ImportStatusRecord(models.Model):
    cid = models.IntegerField(db_column='cID')  # Field name made lowercase.
    status = models.CharField(max_length=10)
    import_time = models.DateTimeField()
    curr_sql_file = models.CharField(max_length=50)
    duration = models.CharField(max_length=10, blank=True, null=True)
    tried_num = models.IntegerField(blank=True, null=True)
    exception_type = models.IntegerField(blank=True, null=True)
    curr_sql_date = models.DateField(blank=True, null=True)
    id = models.AutoField(primary_key=True)

    class Meta:
        managed = False
        db_table = 'import_status_record'

    def __unicode__(self):
        return u'%s %s %s %s %s'%(self.cid,self.status,self.curr_sql_file,self.duration,self.tried_num)
